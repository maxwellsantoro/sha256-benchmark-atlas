/* c-botan: atlas SHA-256 runner */
#include <botan/ffi.h>
static int do_hash(const unsigned char *msg, size_t len, unsigned char out[32]) {
    botan_hash_t hash = NULL;
    size_t out_len = 32;
    if (botan_hash_init(&hash, "SHA-256", 0) != 0) return 1;
    if (botan_hash_update(hash, msg, len) != 0) { botan_hash_destroy(hash); return 1; }
    if (botan_hash_final(hash, out) != 0) { botan_hash_destroy(hash); return 1; }
