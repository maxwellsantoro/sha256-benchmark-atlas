/* c-cryptopp: atlas SHA-256 runner */
#if __has_include(<cryptopp/sha.h>)
#include <cryptopp/sha.h>
#elif __has_include(<crypto++/sha.h>)
#include <crypto++/sha.h>
#else
#error "Crypto++ headers not found (cryptopp/sha.h or crypto++/sha.h)"
#endif
#include <cstring>
...
static int cmd_hash(void) {
    ...
    for (;;) {
        if (len + 65536 > cap) {
            cap = cap ? cap * 2 : 65536;
            unsigned char *nbuf = static_cast<unsigned char *>(realloc(buf, cap));
...
static int cmd_bench(size_t size, uint64_t iters, uint64_t seed) {
    unsigned char *buf = static_cast<unsigned char *>(calloc(1, size ? size : 1));
    unsigned char digest[32];
