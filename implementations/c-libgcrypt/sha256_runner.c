/* c-libgcrypt: atlas SHA-256 runner */
#include <gcrypt.h>
static int do_hash(const unsigned char *msg, size_t len, unsigned char out[32]) {
    if (!gcry_check_version(GCRYPT_VERSION)) return 1;
    gcry_md_hd_t hd;
    if (gcry_md_open(&hd, GCRY_MD_SHA256, 0)) return 1;
    gcry_md_write(hd, msg, len);
    memcpy(out, gcry_md_read(hd, GCRY_MD_SHA256), 32);
    gcry_md_close(hd);
    return 0;
}
