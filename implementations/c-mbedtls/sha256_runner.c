...
static int ensure_psa(void) {
    if (psa_ready) return 0;
    if (psa_crypto_init() != PSA_SUCCESS) return 1;
    psa_ready = 1;
    return 0;
}

static int hex_digest(const unsigned char *d, size_t n) {
...
static int do_hash(const unsigned char *msg, size_t len, unsigned char out[32]) {
    if (ensure_psa()) return 1;
    size_t olen = 0;
    psa_status_t st = psa_hash_compute(PSA_ALG_SHA_256, msg, len, out, 32, &olen);
    return (st != PSA_SUCCESS || olen != 32) ? 1 : 0;
}
