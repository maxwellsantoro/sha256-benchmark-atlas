/* c-nss: atlas SHA-256 runner */
#include <nss.h>
#include <pk11pub.h>
#include <sechash.h>
static int do_hash(const unsigned char *msg, size_t len, unsigned char out[32]) {
    /* NSS wants a one-time init; ignore "already initialized". */
    (void)NSS_NoDB_Init(".");
    unsigned int out_len = 0;
    if (HASH_HashBuf(HASH_AlgSHA256, out, (unsigned char *)msg, (PRUint32)len) != SECSuccess)
        return 1;
    (void)out_len;
    return 0;
}
